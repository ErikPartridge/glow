...
def init_visualizations(hps, model, logdirs):
    ...
    def draw_samples(epoch):
        ...
        for i in range(len(x_samples)):
            ...
            zeus.graphics.save_raster(
                x_sample,
                logdirs[0] +
                'epoch_{}_sample_{}.png'.format(
                    epoch,
                    i))
...
def get_data(hps, sess):
    ...
    print(
        "Rank {} Batch sizes Train {} Test {} Init {}".format(
            hvd.rank(),
            hps.local_batch_train,
            hps.local_batch_test,
            hps.local_batch_init))
    ...
    if hps.problem in [
        ...
        train_iterator, test_iterator, data_init = v.get_data(sess, hps.data_dir, hvd.size(), hvd.rank(
        ), hps.pmap, hps.fmap, hps.local_batch_train, hps.local_batch_test, hps.local_batch_init, hps.image_size, hps.rnd_crop)

    elif hps.problem in ['mnist', 'cifar10']:
        hps.direct_iterator = False
        import data_loaders.get_mnist_cifar as v
        train_iterator, test_iterator, data_init = v.get_data(hps.problem, hvd.size(), hvd.rank(
        ), hps.dal, hps.local_batch_train, hps.local_batch_test, hps.local_batch_init, hps.image_size)
...
def main(hps):
    ...
    if hvd.rank() == 0:
        ...
        _print(
            'epoch n_processed n_images pps dtrain dtest dsample dtot train_results test_results msg')
    ...
    for epoch in range(1, hps.epochs):
        ...
        if epoch < 10 or (epoch < 50 and epoch %
                          10 == 0) or epoch % hps.epochs_full_valid == 0:
            test_results = []
            msg = ''
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Epochs between full scale sample")
