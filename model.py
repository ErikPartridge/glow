    ...
    if hps.direct_iterator:
        ...
        def _train(_lr):
            ...
            return sess.run([train_op, stats_train], {
                            feeds['x']: _x, feeds['y']: _y, lr: _lr})[1]
        m.train = _train
...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    with tf.name_scope('input'):
        X = tf.placeholder(
            tf.uint8, [
                None, hps.image_size, hps.image_size, 3], name='image')
    ...
    def postprocess(x):
        return tf.cast(tf.clip_by_value(
            tf.floor((x + .5) * hps.n_bins) * (256. / hps.n_bins), 0, 255), 'uint8')
