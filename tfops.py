...
def print_act_stats(x, _str=""):
    ...
    stats = [
        tf.reduce_min(x_mean),
        tf.reduce_mean(x_mean),
        tf.reduce_max(x_mean),
        tf.reduce_min(
            tf.sqrt(x_var)),
        tf.reduce_mean(
            tf.sqrt(x_var)),
...
def actnorm_scale(
    ...
    with tf.variable_scope(name), arg_scope([get_variable_ddi], trainable=trainable):
        ...
        if True:
            ...
            # Alternative, doesn't seem to do significantly worse or better
            # than the logarithmic version above
...
# 2X nearest-neighbour upsampling, also inspired by Jascha
# Sohl-Dickstein's code
...
def shuffle_features(
    ...
    with tf.variable_scope(name):
        ...
        tf_indices = tf.get_variable(
            "indices", dtype=tf.int32, initializer=np.asarray(
                indices, dtype='int32'), trainable=False)
        tf_indices_reverse = tf.get_variable(
            "indices_inverse", dtype=tf.int32, initializer=np.asarray(
...
def embedding(name, y, n_y, width):
    with tf.variable_scope(name):
        params = tf.get_variable(
            "embedding", [
                n_y, width], initializer=default_initializer())
