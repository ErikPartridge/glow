...
def centre_crop(img):
    ...
    return tf.image.crop_to_bounding_box(
        img, h_offset, w_offset, min_side, min_side)
        ...
        if split:
            ...
            dset = files.apply(
                tf.contrib.data.parallel_interleave(
                    ...
                    cycle_length=_NUM_PARALLEL_FILE_READERS))
            transpose = False
        else:
        ...
        for lod in range(self.resolution_log2 - 1):
            ...
            for shard in range(shards):
                ...
                tfr_file = self.tfr_prefix + \
                    '-r%02d-s-%04d-of-%04d.tfrecords' % (self.resolution_log2 - lod, shard, shards)
            ...
            print(
                "Smallest and largest shards have size",
                np.min(counts),
                np.max(counts))
